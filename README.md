...
Клава и мышь — полный RealTime, это значит что если событие пришло то оно попадёт в буфер если вы заняты чем то иным, как использовать в RealTime когда вьюпорт закреплён на холсте (F2 по умолчанию - настраивается) то можно перехватить управление полностью - опрашивать не через чтение буфера (дочитали и он пуст) а как это делает вьюпорт напрямую (key) и если список управляющих команд будет не пуст то в определённом режиме они не попадают в буфер а возвращаются напрямую, мышь в принципе живёт в реальности постоянно и вы можете просто считывать то что есть из определённых ячеек так как key комбинированная функция читающая и мышь и клавиатуру.

Клавиатурный буфер на 255/510 символов (255 [3,4 байта UTF8] 510 [1,2 байта UTF8]). Сон на клавиатуре займёт одну ячейку в буфере - но максимум выдаст в результате 255 одинаковых - причём из буфера они передаются разом за один запрос с указанием числа нажатий. Можете реально проверить свои операционный системы - пример: NixOS — идеально, хоть и systemd (мне он не по душе, но NixOS выбран осознанно) - нажал управление вьюпотром — круг (65536 значений меньше минуты) без зависаний пролетел. Жена забрала комп, пришлось на бук поставить Devuan — нравился, но там нажал — и счётчик положения вьюпорта в какой-то момент подвис.

Мышь: использую X10, потому что это единственная реализация мыши, которая уважает стандарт. И да — с этого момента любая мышь работающая по стандарту обретает горизонтальный скролл, надеюсь у неё есть обычное колесо.
