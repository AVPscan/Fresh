...
void InitVram(Cell addr, Cell size) { if (!addr || (size < SizeVram)) return;
  ...
  PalBuf *pal, *mode, *src; char *base, *cbase[] = { "\3\33[3", "\6\33[38;5", "\6\33[38;2" }; uint32_t in, t, p = 256;
  char* modes[] = { "\12;22;23;27m", "\11;22;23;7m", "\11;23;27;1m", "\10;23;1;7m", "\11;22;27;3m", "\10;22;3;7m", "\10;27;1;3m", "\7;1;3;7m" };
  uint8_t i, j, ct, c, k = On, x = 2, h[] = { 0, 4, 1, 5, 2, 6, 3, 7 }; if (CFDeep < b3) { k--; p = b3; } else if (CFDeep > b3) { k++; p = 1 << 24; }
  while(x--) { c = b3; while(c--) { ct = (x) ? h[c] : c; in = (ct) ? (((p * ct) / 7) - On) : Off; if (!k && ct && ct != 7) in++;
