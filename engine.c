...
void Print(uint8_t n, char *str) { if (!n || !str) return;
  char *dst = Cdbuf; ugoc len = StrLen(str); n &= ~b7; PalBuf* pal = APal(n); MemCpy(dst, pal->d, pal->l); dst += pal->l;
  MemCpy(dst, str, len); SysWrite(Cdbuf, dst + len - Cdbuf); }
void BPrint(uint8_t n, char *str) { if (!str) return;
  char *dst = Cdbuf; ugoc len = StrLen(str); n &= b3210; PalBuf* pal = AFon(n); MemCpy(dst, pal->d, pal->l); dst += pal->l;
...
void InitVram(Cell addr, Cell size) { if (!addr || (size < SizeVram)) return;
  PalBuf *pal, *mode, *src; uint32_t in, t;
  uint8_t i, j, ct, c, k = On, p = b3, z = 16, y = z - On, x = 2, h[] = { 0, 1, 8, 14, 2, 4, 9, 6, 3, 8, 11, 10, 5, 12, 13, 15 };
  ...
  while(x--) { c = z;
    while(c--) { src = AFon(c); mode = (PalBuf*)cbase[k];
      ct = (x) ? h[c] : c; in = (ct) ? (uint32_t)((((1 << p) * ct) / y) - On) : Off; if (!k && ct && ct != y) in++;
    ...
    while(c--) { src = AFon(c); i = b3;
      while(i--) { pal = APal((i << b2) + c); mode = (PalBuf*)modes[i]; pal->l = src->l + mode->l - On;
...
void WSwitch(void) { if (Win(VP.Wec)->Xr) Win(VP.Wec)->WF ^= b7; }
