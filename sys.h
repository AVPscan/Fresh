...
typedef struct { anu Error, Loop, Count, Goc, PCell, D, DS, O, V, Attr, CellP, Colours, Deep, Dynamic; vanu On, Apm, Hz, FTime, Rnd, Last, AF, Ink,
  ...
  Sam, HourInDay, MinInHour, SecInMin, PlainVis, PlainYear, CycleYears, YearLength, ShiftYear, EpochShift, DaySec, DaysInWeek, MonInYear, ShiftMonth; } Base_;
...
typedef struct {
  anu col     : 8; // бит 7-0  код цвета {максимум 255 цветовых оттенка 0 чёрный}
} palet;
typedef struct {
...
nanu MemCmp(void* dst, void* src, As len);                   // Сравнение
...
nanu Fsin(vnanu u);                                          // Синус      полный круг 360 градусов [0...511] шаг ~0,7 градуса
...
void GenRGB(anu mode, vanu c, vanu n);                       // Сгенерировать RGB позиции (с) из диапазона до (n) включительно методом (Off/On)
void GenLast(vnanu c);                                       // Сгенерировать цвет и фон, по углу, в позицию last текущей палитры методом sinus
void GenPalettes(void);                                      // Автогенерация оттенков света в палитры двумы методами
void ColourInit(vanu c, vanu d);                             // Установка переменных цвета Colours Deep
As HowSize(anu c, anu d, vanu w, As a);                      // Расчёт общего размера среды
void InitVram(anu c, anu d, vanu w, vanu h, vanu z, vanu a); // Инициализация мира CellPower Dynamic Win How Hz Apm
As SystemSwitch(void);                                       // Вход/выход в мир
void MoveNorm(vgoc x, vgoc y);                               // Нормализация перемещения
void MoveConvas(vgoc dx, vgoc dy);                           // Взаимосвязь перемещения по холсту и экранных координат
anu MoveScreen(vgoc mx, vgoc my);                            // Взаимосвязь изменения экранных координат(мышью) и холста
...
goc Real(rgoc fps);                                          // Сколько реально прошло в ожидании
