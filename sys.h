...
#define Cls         "\033[2J"                 // Очистить экран
...
#define Fcolour   7                           // Количество оттенков света на старте {максимум 127} 0 всегда есть {2 палитры}
...
typedef struct { ugoc Ginf, Gmin, Gmax, A, B, Rnd; uint16_t MWin, Time[5], Syn, Loop, Delay, Spd0, Spd1, Speed, Hz;
...
void SysInit(uint16_t hz, uint8_t deep, uint8_t col);                 // Установка переменных среды
...
void _SSet(uint16_t hz, uint8_t count, uint8_t *args);                // Изменить hz,fps{,deep{,colours}}
...
#define SysSet(hz, ...) _SSet(hz, (uint8_t)((sizeof((uint8_t[]){0, ##__VA_ARGS__}) / sizeof(uint8_t)) - 1), (uint8_t[]){0, ##__VA_ARGS__} + 1)
