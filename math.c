...
typedef struct { anu Nim,   // Единственный рычаг знаковости (0 - беззнаковое). Типов данных НЕТ. Тип - воля создателя!
  Carry, Riz, Rnim, Eiz, Enim, Long, Fa, Fb, Za, Zb, Br, Ba, Bb, *r, *a, *b, *e; MatBuf Ho, Lo, Sr; } var_;
var_ Mat = {.Nim = 0};
...
void Mul (anu l, anu *r, anu *a, anu *b) { if (l) { Mat.Fa = 0; Mat.Fb = 0; Mat.Ba = 1; Mat.Bb = 1; Mat.Long = l;
    ...
    if (Mat.Ba < 2 || Mat.Bb < 2) { if (!Mat.Ba || !Mat.Bb) { Mat.Riz++; Mat.Rnim = 0; return; }
      if (Mat.Nim && (*(Mat.a - 1) == 0x80 || *(Mat.b - 1) == 0x80)) { *r = 0x80; Mat.Riz++; Mat.Rnim = 0xFF; return; } }
