...
void Fror (anu l, anu *r, anu *a) { while(l--) { Mat.Za = (*r = *a++) & 1; *r >>= 1; if (Mat.Carry) *r |= 0x80;
  Mat.Carry = Mat.Za; r++; } }

void Frol (anu l, anu *r, anu *a) { r += l; a += l; while(l--) { Mat.Za = (*--r = *--a) & 0x80; *r <<= 1;
  if (Mat.Carry) { *r |= 1; } Mat.Carry = Mat.Za; } }
