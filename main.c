...
void body(void) {  // пока не дописано WinData,Render
  ...
  while(i--) { *p++ = (VP.Mode & (1 << i)) ? '1' : '0'; } snprintf(p, 91, " %ld Kbyte %d c%d r%d ", s, Base.CellP, TS.c, TS.r); i = (uint8_t)StrLen(b);
...
void Init(void) { GlobalSet(11, 1000, 500, 200); SysSet(Off, 24, 7);
  ugoc control = Window(On,coral, -2, -2), W1 = Window(Off,Rand(Sys.Colours), Off, Off, 80, 24);
  WinData(control, " %+5dMb %+3 %06c:%06c ", ((VRam.size + 1048575)/1048576), moss, moss);
  WinExecs(control, K_Ctrl_K, WSwitch); WinExecs(control, K_NO, body); WinSet(W1, On, On); WinView(W1, 10, 3);
  Events(' ', Timer, K_ALT_TAB, K_ALT_ENT); Execs(sb, tim, WinDown, WinUp);
  SKeys(K_F1,K_Ctrl_DOW,K_Ctrl_LEF,K_Ctrl_UP,K_Ctrl_RIG,K_UP,K_LEF,K_DOW,K_RIG); Sys.Win = W1; }
...
Cell Help(Cell argc, char *argv[], Cell flag) {
  if (argc > On) { if (MemCmp(argv[On], "-?", 2) == Off || MemCmp(argv[On], "-h", 2) == Off || MemCmp(argv[On], "-help", 5) == Off) {
      if (flag) { Print(Sys.Fone, Off, "\033[?1049l"); Print(coral, aB | aC, " Created by Alexey Pozdnyakov"); flag = Off;
        Print(berry, aC, " in 07.02.2026 version 9.88 email: avp70ru@mail.ru https://github.com/AVPscan \033[0m\n"); } } } return flag; }
