...
void body(void) {  // пока не дописано WinData,Render
  char *p = Cdbuf + 512, *b = p; uint8_t l, v, w, i = Fcolour; if (i > b5) { i = b5; } else { if (i < b1) i = b1; } w = i - On;
  ...
  if (StrLen(b) > 43) { *(b + 43) = Off; } if (StrLen(b) >= c) { *(b + c - On) = Off; } Print(olive,b); if (r < 3) return;
  snprintf(b, 100, "\nx%d y%d %d %d b%d x%d y%d                           ", VP.X, VP.Y, VP.Xs, VP.Ys, Buf.Mkey, Buf.MX, Buf.MY);
  if (StrLen(b) > 44) { *(b + 44) = Off; } if (StrLen(b) >= c) { *(b + c) = Off; } Print(marsala,b); if (r < 4) return;
  ...
  snprintf(p, 30, "                   "); if (StrLen(b) > 44) { *(b + 44) = Off; } if (StrLen(b) >= c) { *(b + c) = Off; } if (r > 3) Print(navy,b); } }

//void body(void) { WinData(VP.Wec, "%1b%2d%3d", VP.Mode, VP.X, VP.Y); }
void sb(void) { if (++Convas.Border >= Fcolour) { Convas.Border = Off; } BPrint(Convas.Border,Cls); }
void Init(void) {
...
Cell Help(Cell argc, char *argv[], Cell flag) {
  if (argc > On) {
    if (MemCmp(argv[On], "-?", 2) == Off || MemCmp(argv[On], "-h", 2) == Off || MemCmp(argv[On], "-help", 5) == Off) {
      if (flag) { Print(Convas.Fone,AltBufOff); Print(fuchsia,"Created by Alexey Pozdnyakov"); flag = Off;
        Print(marsala," in 07.02.2026 version 9.10 email: avp70ru@mail.ru https://github.com/AVPscan"); } } } return flag; }
