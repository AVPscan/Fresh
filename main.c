...
void Init(void) {
  ...
  WinSet(W1, On, On); WinSet(W2, Off, Off); WinView(W2, 10, 3); Events(' ', K_ALT_TAB, K_ALT_ENT); Execs(sb, WinDown, WinUp); SysSet(0, 144, 24, 7);
...
Cell Help(Cell argc, char *argv[], Cell flag) {
  if (argc > On) { if (MemCmp(argv[On], "-?", 2) == Off || MemCmp(argv[On], "-h", 2) == Off || MemCmp(argv[On], "-help", 5) == Off) {
      if (flag) { Print(Sys.Fone, cA, AltBufOff); Print(berry, cA, "Created by Alexey Pozdnyakov"); flag = Off;
        Print(coral, cA, " in 07.02.2026 version 9.57 email: avp70ru@mail.ru https://github.com/AVPscan"); } } } return flag; }
