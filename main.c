...
void body(void) {  // пока не дописано WinData,Render
  char *p = var.dbuf + 512, *b = p; uint8_t l, v, j = 48, w = Base.Colours, i = On + w; Print(Sys.Inc, Off, "\033[H");
  ...
  snprintf(b, 100, "\nH%d F%d D%d C%d x%d y%d %d %d b%d x%d y%d                         ", Base.Hz/10, Base.Fps, Base.Deep, Base.Colours, VP.X, VP.Y, VP.Xs, VP.Ys,
...
void Init(void) { GlobalSet(12, Off, 512, 500, 200); SysSet(7, 24, 10); SKeys(K_F1,K_Ctrl_DOW,K_Ctrl_LEF,K_Ctrl_UP,K_Ctrl_RIG,K_UP,K_LEF,K_DOW,K_RIG);
...
Cell Help(Cell argc, char *argv[], Cell flag) {
  if (argc > On) { if (MemCmp(argv[On], "-?", 2) == Off || MemCmp(argv[On], "-h", 2) == Off || MemCmp(argv[On], "-help", 5) == Off) {
      if (flag) { Print(Sys.Fone, Off, "\033[?1049l"); Print(coral, aB | aC, " Created by Alexey Pozdnyakov"); flag = Off;
        Print(berry, aC, " in 07.02.2026 version 9.89 email: avp70ru@mail.ru https://github.com/AVPscan \033[0m\n"); } } } return flag; }
