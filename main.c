...
void body(void) {  // пока не дописано WinData,Render
  ...
  while(i--) { *p++ = (VP.Mode & (1 << i)) ? '1' : '0'; } snprintf(p, 91, " %lu Kbyte %d c%lu r%lu ", (unsigned long)s, K_Mouse, (unsigned long)TS.c, (unsigned long)TS.r); i = (uint8_t)StrLen(b);
...
void Init(void) {
  ugoc control = Window(On,coral, -2, -2), W1 = Window(Off,Rand(Sys.Colours), Off, Off, 80, 24), W2 = Window(Off,berry, Rand(10), Rand(10), Rand(40), Rand(10));
  ...
  WinSet(W1, On, On); WinSet(W2, Off, Off); WinView(W1, 10, 3); Events(' ', Timer, K_ALT_TAB, K_ALT_ENT); Execs(sb, tim, WinDown, WinUp); SysSet(500, 180, Off, 24, 7);
  SKeys(K_F1,K_Ctrl_DOW,K_Ctrl_LEF,K_Ctrl_UP,K_Ctrl_RIG,K_UP,K_LEF,K_DOW,K_RIG); Sys.Win = W2; }
...
Cell Help(Cell argc, char *argv[], Cell flag) {
  if (argc > On) { if (MemCmp(argv[On], "-?", 2) == Off || MemCmp(argv[On], "-h", 2) == Off || MemCmp(argv[On], "-help", 5) == Off) {
      if (flag) { Print(Sys.Fone, var.A, "\033[?1049l"); Print(berry, var.A, "Created by Alexey Pozdnyakov"); flag = Off;
        Print(coral, var.A, " in 07.02.2026 version 9.82 email: avp70ru@mail.ru https://github.com/AVPscan\033[0m\n"); } } } return flag; }
