  ...
  if (argc > 1) {
    if (MemCmp(argv[1], "-?",2) == 0 || MemCmp(argv[1], "-h",2) == 0 || MemCmp(argv[1], "-help",5) == 0) {
      if (flag) { Print(Cdefault,AltBufOff); Print(CorangeBI," Created by Alexey Pozdnyakov "); flag = 0;
                  Print(Corange," in 07.02.2026 version 7.59 email: avp70ru@mail.ru https://github.com/AVPscan\n"); } } }
  return flag; }
...
int main(int argc, char *argv[]) {
  ...
  if (flag) { uint8_t data = Window(Cgreen, 0, 0); WData(data, "Память под Vram %03cdMbytes\n", Corange, (uint16_t)((SizeVram + 1048575)/1048576));
              uint8_t control = Window(Cgold, 0, -1); WConst(control, " %3 %06c:%06c ", Cblue, Cblue); WSet(control, -2, -2);
