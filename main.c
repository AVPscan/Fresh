...
void body(void) {  // пока не дописано WinData,Render
  ...
  if ((uint8_t)StrLen(b) > (j - 1)) { *(b + j - 1) = Off; } if (StrLen(b) >= c) { *(b + c - On) = Off; } Print(marsala,b); if (r < 3) return;
  ...
  if (i || Buf.Count) { l = On + (w & b10); v = ((w>>2) & b10); w = (w & b5) ? On : Off; p = b;
    ...
    snprintf(p, 30, "                     "); if (StrLen(b) > j) { *(b + j) = Off; } if (StrLen(b) >= c) { *(b + c) = Off; } if (r > 3) Print(navy,b); } }
//void body(void) { WinData(VP.Wec, "%1b%2d%3d", VP.Mode, VP.X, VP.Y); }
void sb(void) { if (!++Sys.Border || Sys.Border > (Sys.Colours + aColours + 1)) { Sys.Border = aColours + 1; } Print(Sys.Border,Cls); }
...
Cell Help(Cell argc, char *argv[], Cell flag) { 
  if (argc > On) { if (MemCmp(argv[On], "-?", 2) == Off || MemCmp(argv[On], "-h", 2) == Off || MemCmp(argv[On], "-help", 5) == Off) {
      if (flag) { Print(Sys.Fone,AltBufOff); Print(fuchsia | aCB,"Created by Alexey Pozdnyakov"); flag = Off;
        Print(fuchsia | aC," in 07.02.2026 version 9.41 email: avp70ru@mail.ru https://github.com/AVPscan"); } } } return flag; }
