...
void body(void) {  // пока не дописано WinData,Render
  ...
  if (StrLen(b) > 43) { *(b + 43) = Off; } if (StrLen(b) >= c) { *(b + c + On) = Off; } Print(CCBgold,b); if (r < 2) return;
  ...
  snprintf(p, 30, "                   "); if (StrLen(b) > 44) { *(b + 44) = Off; } if (StrLen(b) >= c) { *(b + c + On) = Off; } if (r > 2) Print(CBgreen,b); } }
  ...
  if (argc > On) {
    if (MemCmp(argv[On], "-?", 2) == Off || MemCmp(argv[On], "-h", 2) == Off || MemCmp(argv[On], "-help", 5) == Off) {
      if (flag) { Print(Cblack,AltBufOff); Print(CBorange,"Created by Alexey Pozdnyakov "); flag = Off;
        Print(Corange,"in 07.02.2026 version 8.91 email: avp70ru@mail.ru https://github.com/AVPscan\n"); } } } return flag; }
