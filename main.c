  ...
  if (argc > 1) {
    if (MemCmp(argv[1], "-?",2) == 0 || MemCmp(argv[1], "-h",2) == 0 || MemCmp(argv[1], "-help",5) == 0) {
      if (flag) { Print(Cconvas,AltBufOff); Print(CorangeIB," Created by Alexey Pozdnyakov "); flag = Off;
                  Print(Corange," in 07.02.2026 version 7.991 email: avp70ru@mail.ru https://github.com/AVPscan\n"); } } }
  return flag; }
...
int main(int argc, char *argv[]) {
  ...
  if (flag) { uint16_t size = (uint16_t)((SizeVram + 1048575)/1048576), control = Window(-Cgold), menu = Window(-CgoldIB), mc = Window(-CgoldIB);
              WinData(menu, "%0mCreate\nLoad\nSave\nExit\n", K_Ctrl_C, K_Ctrl_L, K_Ctrl_S, K_Ctrl_E); WinSet(menu, K_ESC); WinView(control, -2, -2);
              WinData(mc, "%mColour U\nBold B\nInverse N\n", K_Ctrl_U, K_Ctrl_B, K_Ctrl_N); WinSet(mc, K_Ctrl_C, 0, 0, show, 0);
