...
void Body(void) {  // пока не дописано WinData,Render9
  ...
  Print((Base.Colours == 1) ? Ink(0) : Ink(3), aB, b); snprintf(b, 100, "%d %d %ld Time %s                         ", Base.Din, Base.Sap, Base.Sam, Base.T);
...
void Init(void) { Fresh(11, 1, 999, 25, 500, 250); Colour(7, 24); Keys(K_ESC); Even(' ', Timer, K_ALT_TAB, K_ALT_ENT); Exec(Space, Tim, WDown, WUp);
