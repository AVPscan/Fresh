...
void body(void) {  // пока не дописано WinData,Render
  char *p = Cdbuf + 512, *b = p; uint8_t l, v, j = 48, w = Sys.Colours, i = On + w; SetMode(Off); Print(Sys.Inc,Home); while(i--) Print((w - i + aColours + 1)," ");
  ...
  if ((uint8_t)StrLen(b) > (j - 1)) { *(b + j - 1) = Off; } if (StrLen(b) >= c) { *(b + c - On) = Off; } SetMode(aC); Print(Sys.Inc,b); if (r < 3) return;
  snprintf(b, 100, "\nF%d D%d C%d x%d y%d %d %d b%d x%d y%d                   ", Sys.Fps, Sys.Deep, Sys.Colours, VP.X, VP.Y, VP.Xs, VP.Ys, Buf.Mkey, Buf.MX, Buf.MY);
  if (StrLen(b) > j) { *(b + j) = Off; } if (StrLen(b) >= c) { *(b + c) = Off; } SetMode(aB); Print(Sys.Inc,b); if (r < 4) return;
  ...
  if (i || Buf.Count) { l = On + (w & b10); v = ((w>>2) & b10); w = (w & b5) ? On : Off; p = b;
    ...
    l = Sys.Inc; if (Sys.Colours > On) { l = (On + Sys.Colours) >> On; } SetMode(aCBI); Print(l,b); } } }
...
Cell Help(Cell argc, char *argv[], Cell flag) {
  if (argc > On) { if (MemCmp(argv[On], "-?", 2) == Off || MemCmp(argv[On], "-h", 2) == Off || MemCmp(argv[On], "-help", 5) == Off) {
      if (flag) { Print(Sys.Fone,AltBufOff); Print(navy,"Created by Alexey Pozdnyakov"); flag = Off;
        Print(fuchsia," in 07.02.2026 version 9.48 email: avp70ru@mail.ru https://github.com/AVPscan"); } } } return flag; }
