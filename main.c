...
void show(void) { if ((Win(VP.Win)->EF ^= b0) & b0) WinView(VP.Win, -2, -2); }
void Init(void) {
  ...
  WinData(menu, "%0mCreate\nLoad\nSave\nExit", 'c', 'l', 's', 'e'); WinSet(menu, K_ESC, Adaptive, Nop, Nop, Nop, Bye); Exec(K_F11) = Off; Exec(K_F12) = Off;
  WinData(c, "%12mName:", 'n'); WinSet(c, 'c', Adaptive, Nop); VP.Anchor = K_PUP; VP.Exit = K_PDN; Exec(VP.Anchor) = Anchor; Exec(VP.Exit) = Bye; Window(Cconvas); }
  ...
  if (argc > On) {
    if (MemCmp(argv[On], "-?", 2) == Off || MemCmp(argv[On], "-h", 2) == Off || MemCmp(argv[On], "-help", 5) == Off) {
      if (flag) { Print(Cconvas,AltBufOff); Print(CorangeB,"Created by Alexey Pozdnyakov "); flag = Off;
        Print(Corange,"in 07.02.2026 version 8.53 email: avp70ru@mail.ru https://github.com/AVPscan\n"); } } } return flag; }
