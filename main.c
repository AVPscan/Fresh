...
void body(void) {  // пока не дописано WinData,Render
  char *p = var.dbuf + 512, *b = p; uint8_t l, v, w, j = 51, i = 3; Print(var.F, var.A, "\033[H"); Print(var.I, var.A, "");
  Cell s = ((VRam.size + 1023) / 1024); *p++ = 'v'; while(i--) { *p++ = (VP.Mode & (1 << i)) ? '1' : '0'; } 
  snprintf(p, 91, " %ld Kbyte %d %d c%d r%d ", s, Base.CellP, Base.Win, TS.c, TS.r); i = (uint8_t)StrLen(b); Print((8 * Base.Colours) / 15, aB, b);
  snprintf(b, 100, " Time %s             ", Base.T); if ((uint8_t)StrLen(b) > (j - 1 - i)) { *(b + j - 1 - i) = Off; }
  if ((uint8_t)StrLen(b) >= (TS.c - i)) { *(b + TS.c - On - i) = Off; } Print(snow, Off, b); if (TS.r < 3) return;
  snprintf(b, 100, "\nH%d A%d D%d C%d x%d y%d %d %d b%d x%d y%d                         ", Base.Hz/10, Base.Apm, Base.Deep, Base.Colours, VP.X, VP.Y, VP.Xs, VP.Ys,
  Buf.Mkey, Buf.MX, Buf.MY); if (StrLen(b) > j) { *(b + j) = Off; } if (StrLen(b) >= TS.c) { *(b + TS.c) = Off; } Print((12 * Base.Colours) / 15, aC, b);
  if (TS.r < 4) { return; } if (Buf.pop > Buf.push) { i = PopKey(); } else { i = ShowKey(); } w = Buf.Dat; if (i || Buf.Count) { l = On + (w & b10); v = ((w>>2) & b10);
    ...
    if (TS.r > 3) Print(dark, aB, b); } }
//void body(void) { WinData(VP.Wec, "%1fb%d%d%s", VP.Mode, VP.X, VP.Y, Base.T); }
void sb(void) { if (!Base.On) SetBorder(Base.On, var.Br++); }
void tim(void) { static int16_t c = Off; GenLast(++c); SetBorder(Base.On, var.Br); }

void Init(void) { Fresh(10, 500, 10, 500, 250); Colour(7, 24); Keys(K_F1,K_Ctrl_DOW,K_Ctrl_LEF,K_Ctrl_UP,K_Ctrl_RIG,K_UP,K_LEF,K_DOW,K_RIG);
...
void Help(Cell argc, char *argv[], Cell *flag) {
  if (argc > On) { if (MemCmp(argv[On], "-?", 2) == Off || MemCmp(argv[On], "-h", 2) == Off || MemCmp(argv[On], "-help", 5) == Off) {
      if (*flag) { Print(Snow, Off, "\033[?1049l"); Print(dark, aB | aC, " Created by Alexey Pozdnyakov "); *flag = Off;
        Print(dark, aC, "in 07.02.2026 version 10.03 email: avp70ru@mail.ru https://github.com/AVPscan \n"); } } } }
