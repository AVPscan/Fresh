...
void body(void) {  // пока не дописано WinData,Render
  char *p = Cdbuf + 512, *b = p; uint8_t l, v, w, i = Convas.Colours; if (i > aColours) { i = aColours; } else { if (i < b1) i = b1; } w = i - On;
  ...
  if (StrLen(b) > 44) { *(b + 44) = Off; } if (StrLen(b) >= c) { *(b + c) = Off; } Print(cyan,b); if (r < 4) return;
  ...
  snprintf(p, 30, "                   "); if (StrLen(b) > 44) { *(b + 44) = Off; } if (StrLen(b) >= c) { *(b + c) = Off; } if (r > 3) Print(ochre,b); } }

//void body(void) { WinData(VP.Wec, "%1b%2d%3d", VP.Mode, VP.X, VP.Y); }
void sb(void) { if (++Convas.Border >= Convas.Colours) { Convas.Border = Off; } BPrint(Convas.Border,Cls); }
void Init(void) {
  ...
  if (argc > On) {
    if (MemCmp(argv[On], "-?", 2) == Off || MemCmp(argv[On], "-h", 2) == Off || MemCmp(argv[On], "-help", 5) == Off) {
      if (flag) { Print(Convas.Fone,AltBufOff); Print(fuchsia | aCB,"Created by Alexey Pozdnyakov"); flag = Off;
        Print(fuchsia | aC," in 07.02.2026 version 9.20 email: avp70ru@mail.ru https://github.com/AVPscan"); } } } return flag; }
