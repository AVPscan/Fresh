...
void Init(void) { GlobalSet(12, Off, 512, 500, 200); SysSet(7, 24, 20); SKeys(K_F1,K_Ctrl_DOW,K_Ctrl_LEF,K_Ctrl_UP,K_Ctrl_RIG,K_UP,K_LEF,K_DOW,K_RIG);
...
Cell Help(Cell argc, char *argv[], Cell flag) {
  if (argc > On) { if (MemCmp(argv[On], "-?", 2) == Off || MemCmp(argv[On], "-h", 2) == Off || MemCmp(argv[On], "-help", 5) == Off) {
      if (flag) { Print(Sys.Fone, Off, "\033[?1049l"); Print(coral, aB | aC, " Created by Alexey Pozdnyakov"); flag = Off;
        Print(berry, aC, " in 07.02.2026 version 9.90 email: avp70ru@mail.ru https://github.com/AVPscan \033[0m\n"); } } } return flag; }
