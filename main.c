...
void body(void) {  // пока не дописано WinData,Render
  char *p = Cdbuf + 512, *b = p; uint8_t l, v, w, i = 16; ugoc r, c = TermCR(&r), s = (ugoc)((VRam.size + 1048575) / 1048576); *p++ = 'v';
  Print(Convas.Fone,Home); while(i--) { BPrint(i," "); } Print(Convas.Fone,"\n"); i = 8;
...
void sb(void) { Convas.Border++; Convas.Border &= b3210; BPrint(Convas.Border,Cls); }
void Init(void) {
  ...
  if (argc > On) {
    if (MemCmp(argv[On], "-?", 2) == Off || MemCmp(argv[On], "-h", 2) == Off || MemCmp(argv[On], "-help", 5) == Off) {
      if (flag) { Print(black,AltBufOff); Print(navyB,"Created by Alexey Pozdnyakov "); flag = Off;
        Print(navyC,"in 07.02.2026 version 8.99 email: avp70ru@mail.ru https://github.com/AVPscan\n"); } } } return flag; }
