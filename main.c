...
void Init(void) {
  ...
  WinSet(W1, On, On); WinSet(W2, Off, Off); WinView(W2, 10, 3); Events(' ', Timer, K_ALT_TAB, K_ALT_ENT); Execs(sb, tim, WinDown, WinUp); SysSet(500, 120, 24, 7, 1);
...
Cell Help(Cell argc, char *argv[], Cell flag) {
  if (argc > On) { if (MemCmp(argv[On], "-?", 2) == Off || MemCmp(argv[On], "-h", 2) == Off || MemCmp(argv[On], "-help", 5) == Off) {
      if (flag) { Print(Sys.Fone, cA, "\033[?1049l"); Print(berry, cA, "Created by Alexey Pozdnyakov"); flag = Off;
        Print(coral, cA, " in 07.02.2026 version 9.70 email: avp70ru@mail.ru https://github.com/AVPscan"); } } } return flag; }
